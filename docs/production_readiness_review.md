...
| Area | Status | Notes |
| --- | --- | --- |
| Architecture & Deployment | ✅ Ready | FastAPI TLS client bundles, the Secrets Service encryption key, and the backing service DSNs now reconcile from Vault-backed ExternalSecrets, the FastAPI settings point at the TLS-enabled Kafka, NATS, TimescaleDB, Redis, and Feast endpoints, and cert-manager definitions provision the ingress certificates, so rollouts no longer require manual secret alignment. 【F:deploy/k8s/base/fastapi/deployments.yaml†L27-L116】【F:deploy/k8s/base/secrets/fastapi-tls-external-secrets.yaml†L1-L203】【F:deploy/k8s/base/secrets/secrets-service-config-external-secret.yaml†L1-L21】【F:deploy/k8s/base/fastapi/configmap.yaml†L12-L42】【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L126】【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L230】【F:deploy/k8s/base/aether-services/deployment-capital-allocator.yaml†L24-L84】 |
| Reliability & Observability | ✅ Ready | Drift promotion metrics, OMS gateway telemetry, Kraken ingest freshness, and kill-switch state feed the shipped alert rules, Prometheus static targets align with overlay suffixes, dedicated ServiceMonitors collect the annotated APIs, and Fluent Bit ships and retains logs centrally, restoring observability parity. 【F:metrics.py†L421-L457】【F:metrics.py†L688-L693】【F:metrics.py†L1302-L1403】【F:oms_service.py†L128-L178】【F:drift_service.py†L156-L229】【F:services/kraken_ws_ingest.py†L48-L120】【F:kill_switch.py†L26-L175】【F:deploy/observability/prometheus/configmap.yaml†L15-L48】【F:deploy/observability/prometheus/servicemonitors.yaml†L1-L91】【F:deploy/observability/prometheus/prometheus.yaml†L33-L120】【F:deploy/observability/logging/fluent-bit-daemonset.yaml†L1-L101】 |
| Security & Compliance | ✅ Ready | Network policies and Dockerfiles are hardened, and the Secrets Service key plus every Kraken API credential (`kraken-keys-*`) reconcile from Vault through ExternalSecrets. Kafka, NATS, and their ingress services now terminate TLS with Vault-provisioned certificates, eliminating the plaintext listener gap noted previously. 【F:deploy/k8s/networkpolicy.yaml†L1-L112】【F:deploy/docker/risk-api/Dockerfile†L1-L26】【F:deploy/k8s/base/secrets/secrets-service-config-external-secret.yaml†L1-L16】【F:deploy/k8s/base/secrets/kraken-keys-external-secrets.yaml†L1-L44】【F:deploy/k8s/base/aether-services/deployment-oms.yaml†L92-L124】【F:deploy/k8s/base/kafka-nats/stack.yaml†L1-L141】【F:docs/runbooks/secrets-service-key-rotation.md†L1-L112】 |
...
* RMT-028 — Point Feast serving at the dedicated Feast Redis service (e.g., the shared `redis` endpoint) or remove the unused `redis-feast` stack so online feature reads hit the correct backend. Files: `deploy/k8s/base/feast/configmap.yaml`, `deploy/k8s/base/feast/deployment.yaml`, cleanup removing the legacy manifests. Severity: High. Owner: Data Platform. Status: Mitigated (cleanup complete).

## Reliability & Observability

* The Prometheus rule set queries histograms and counters named `oms_order_latency_seconds`, `policy_latency_ms`, `risk_latency_ms`, `pricing_service_mid_price_delta`, and `ws_delivery_latency_seconds`. Instrumentation now exports the latency histograms, the OMS gateway counters, and the WebSocket latency and freshness metrics so the related alerts evaluate against live data. 【F:deploy/observability/prometheus/prometheus.yaml†L33-L150】【F:metrics.py†L421-L693】【F:metrics.py†L1302-L1403】【F:services/kraken_ws_ingest.py†L48-L120】
