...
* `fastapi-config` now pins the Kafka, NATS, TimescaleDB, Redis, and Feast endpoints to the cluster TLS listeners so FastAPI pods negotiate encrypted connections with the backing services. 【F:deploy/k8s/base/fastapi/configmap.yaml†L12-L34】
