...
| Reliability & Observability | ❌ Needs Fix | Prometheus rules still rely on the unimplemented order-gateway counters and model canary histogram, but latency instrumentation has been aligned with the alert names: the shared helper now exports `policy_latency_ms`, `risk_latency_ms`, and `oms_order_latency_seconds`, and the Kraken ingestor publishes the `ws_delivery_latency_seconds` histogram alongside the market-data freshness gauge so latency and freshness alerts ingest live series. Scrape configuration follows the `prometheus.io/scrape=true` annotations and static targets match suffixed Service names, while kill-switch state metrics back the runbook guidance. Additional ServiceMonitors and exporter coverage remain required for alerting parity. Centralized log rotation configured; disk exhaustion risk mitigated. 【F:deploy/observability/prometheus/prometheus.yaml†L33-L176】【F:deploy/observability/prometheus/configmap.yaml†L11-L48】【F:deploy/k8s/overlays/production/kustomization.yaml†L1-L18】【F:deploy/k8s/overlays/staging/kustomization.yaml†L1-L18】【F:metrics.py†L538-L631】【F:services/kraken_ws_ingest.py†L40-L216】【181bac†L1-L2】【F:kill_switch.py†L26-L175】【F:deploy/k8s/base/aether-services/deployment-risk.yaml†L1-L80】【F:deploy/k8s/base/aether-services/deployment-secrets.yaml†L1-L80】【F:deploy/k8s/base/kraken-ws-ingest/deployment.yaml†L1-L44】【F:deploy/k8s/base/kraken-ws-ingest/service.yaml†L1-L14】【F:docs/runbooks/kill_switch_activation.md†L1-L34】【F:docs/runbooks/websocket_desync.md†L1-L38】【6f6b28†L1-L2】【4ecd18†L1-L2】【a8ce36†L1-L2】【b12eb7†L1-L3】【F:deploy/observability/logging/fluent-bit-daemonset.yaml†L1-L76】 |
