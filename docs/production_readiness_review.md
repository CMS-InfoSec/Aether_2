...
| Area | Status | Notes |
| --- | --- | --- |
| Architecture & Deployment | ✅ Mitigated | FastAPI TLS client secrets and the Secrets Service encryption key now reconcile from Vault-backed ExternalSecrets, unblocking the API rollouts, and Feast still targets the dedicated Redis master with complete credentials. Cert-manager Certificates back every ingress TLS secret (`aether-risk-tls` plus the per-service `*-service-tls` bundle), so HTTPS promotion no longer depends on hand-crafted secrets. 【F:deploy/k8s/base/fastapi/deployments.yaml†L27-L114】【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L439】【F:deploy/k8s/base/secrets/fastapi-tls-external-secrets.yaml†L1-L66】【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L84】【F:deploy/k8s/base/aether-services/ingress-risk.yaml†L1-L26】 |
| Reliability & Observability | ✅ Mitigated | The drift service now emits the `model_canary_promotion_duration_minutes` histogram so SLO alerts and dashboards ingest live promotion durations alongside the OMS gateway, latency, and freshness metrics. Prometheus now ships ServiceMonitors for each FastAPI workload and restores the scaling-controller alert rules referenced in runbooks, closing the alerting coverage gap. Centralized log rotation configured; disk exhaustion risk mitigated. 【F:deploy/observability/prometheus/prometheus.yaml†L33-L176】【F:deploy/observability/prometheus/servicemonitors.yaml†L1-L70】【F:deploy/k8s/overlays/production/kustomization.yaml†L1-L18】【F:deploy/k8s/overlays/staging/kustomization.yaml†L1-L18】【F:metrics.py†L421-L457】【F:metrics.py†L688-L693】【F:metrics.py†L1302-L1403】【F:oms_service.py†L108-L176】【F:services/kraken_ws_ingest.py†L40-L216】【F:drift_service.py†L149-L229】【181bac†L1-L2】【F:kill_switch.py†L26-L175】【F:deploy/k8s/base/aether-services/deployment-risk.yaml†L1-L80】【F:deploy/k8s/base/aether-services/deployment-secrets.yaml†L1-L80】【F:deploy/k8s/base/kraken-ws-ingest/deployment.yaml†L1-L60】【F:deploy/k8s/base/kraken-ws-ingest/service.yaml†L1-L14】【F:docs/runbooks/kill_switch_activation.md†L1-L34】【F:docs/runbooks/websocket_desync.md†L1-L38】【6f6b28†L1-L2】【4ecd18†L1-L2】【a8ce36†L1-L2】【b12eb7†L1-L3】【F:deploy/observability/logging/fluent-bit-daemonset.yaml†L1-L76】 |
| Security & Compliance | ✅ Mitigated | Network policies and Dockerfiles are hardened, and the Secrets Service key plus every Kraken API credential (`kraken-keys-*`) are now delivered from Vault through ExternalSecrets, so GitOps covers the high-risk credentials. Kafka listeners now serve TLS from the GitOps-managed certificate while Zookeeper enforces SASL credentials sourced via ExternalSecrets, eliminating the anonymous plaintext access path. 【F:deploy/k8s/networkpolicy.yaml†L1-L112】【F:deploy/docker/risk-api/Dockerfile†L1-L26】【F:deploy/k8s/base/secrets/secrets-service-config-external-secret.yaml†L1-L16】【F:deploy/k8s/base/secrets/kraken-keys-external-secrets.yaml†L1-L44】【F:deploy/k8s/base/aether-services/deployment-oms.yaml†L92-L124】【F:deploy/k8s/base/kafka-nats/stack.yaml†L24-L120】【F:deploy/k8s/base/secrets/external-secrets.yaml†L439-L462】 |
...
* `fastapi-config` now references the TLS bootstrap endpoints for Kafka alongside the existing service DNS names, and pods mount the Vault-rendered CA bundle so clients negotiate SSL successfully. 【F:deploy/k8s/base/fastapi/configmap.yaml†L12-L34】【F:deploy/k8s/base/fastapi/deployment-risk.yaml†L37-L57】
* The risk ingress consumes the GitOps-managed `aether-risk-tls` Certificate, eliminating the need for manually created TLS secrets during promotion. 【F:deploy/k8s/base/ingress/ingress.yaml†L27-L41】【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L84】
* Every service-specific ingress (`auth|fees|oms|policy|risk|secrets|universe`) now reconciles a corresponding cert-manager Certificate, ensuring HTTPS secrets are provisioned automatically. 【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L84】
...
* RMT-013 — Create a GitOps-managed TLS certificate (`aether-risk-tls`) for the risk ingress instead of relying on a manually provisioned secret. Files: `deploy/k8s/base/ingress/ingress.yaml`, `deploy/k8s/base/secrets/`. Severity: High. Owner: Platform. Status: Mitigated.
...
* RMT-027 — Define certificates/ExternalSecrets for each ingress TLS secret (`auth|fees|oms|policy|risk|secrets|universe` `*-service-tls`) so cert-manager can provision HTTPS for every service. Files: `deploy/k8s/base/aether-services/ingress-*.yaml`, secrets overlays. Severity: High. Owner: Platform. Status: Mitigated.
