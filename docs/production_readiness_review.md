...
| Area | Status | Notes |
| --- | --- | --- |
| Architecture & Deployment | ✅ Ready | FastAPI TLS client secrets and the Secrets Service encryption key reconcile from Vault-backed ExternalSecrets, the shared services continue to point at the dedicated Redis master, and cert-manager now provisions `aether-risk-tls` plus every `*-service-tls` secret via the committed Certificate manifests so ingress HTTPS is fully automated. 【F:deploy/k8s/base/fastapi/deployments.yaml†L27-L114】【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L248】【F:deploy/k8s/base/secrets/fastapi-tls-external-secrets.yaml†L1-L140】【F:deploy/k8s/base/secrets/secrets-service-config-external-secret.yaml†L1-L16】【F:deploy/k8s/base/fastapi/configmap.yaml†L12-L34】【F:deploy/k8s/base/feast/external-secret.yaml†L1-L18】【F:deploy/k8s/base/feast/deployment.yaml†L33-L86】【F:deploy/k8s/base/feast/configmap.yaml†L9-L21】【F:deploy/k8s/base/redis-feast/deployments.yaml†L1-L120】【F:deploy/k8s/base/aether-services/deployment-capital-allocator.yaml†L1-L85】【F:deploy/k8s/base/ingress/ingress.yaml†L1-L41】【F:deploy/k8s/base/aether-services/ingress-auth.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-fees.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-oms.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-policy.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-risk.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-secrets.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-universe.yaml†L1-L33】【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L104】 |
...
* The risk ingress and every service-specific ingress (`auth|fees|oms|policy|risk|secrets|universe`) remain annotated for the LetsEncrypt ClusterIssuer, and the committed `tls-certificates.yaml` file now provisions `aether-risk-tls` plus each `*-service-tls` secret through cert-manager so HTTPS is GitOps-managed. 【F:deploy/k8s/base/ingress/ingress.yaml†L1-L41】【F:deploy/k8s/base/aether-services/ingress-auth.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-fees.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-oms.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-policy.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-risk.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-secrets.yaml†L1-L33】【F:deploy/k8s/base/aether-services/ingress-universe.yaml†L1-L33】【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L104】
