...
* TLS configuration in `fastapi-config` references Kubernetes secrets for Kafka, NATS, TimescaleDB, Redis, and Feast client certificates (`kafka-tls`, `timescaledb-client-cert`, etc.), and those secrets now reconcile from Vault-backed ExternalSecrets even though the backing services still expose only plaintext listeners. 【F:deploy/k8s/base/fastapi/configmap.yaml†L12-L49】【F:deploy/k8s/base/secrets/fastapi-tls-external-secrets.yaml†L1-L140】
