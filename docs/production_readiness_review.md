...
| Area | Status | Notes |
| --- | --- | --- |
| Architecture & Deployment | ❌ Needs Fix | FastAPI workloads reference `fastapi-secrets` keys that are not rendered, the Secrets Service encryption key is managed out-of-band, the runtime expects TLS secrets (`kafka-tls`, `timescaledb-client-cert`, `aether-risk-tls`, etc.) that have no manifests, and every service ingress (`*-service-tls`) references TLS secrets the repository never defines. Feast now reconciles its offline-store credentials with both username and password, targets the dedicated Redis master service, and Vault-backed ExternalSecrets deliver the compliance and capital allocator database DSNs, but the remaining TLS gaps still block promotion. 【F:deploy/k8s/base/fastapi/deployments.yaml†L27-L114】【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L248】【F:deploy/k8s/base/aether-services/secret-secrets-service-config.yaml†L1-L6】【F:deploy/k8s/base/fastapi/configmap.yaml†L12-L49】【37778a†L1-L7】【F:deploy/k8s/base/feast/external-secret.yaml†L1-L18】【F:deploy/k8s/base/feast/deployment.yaml†L33-L86】【F:deploy/k8s/base/feast/configmap.yaml†L9-L21】【F:deploy/k8s/base/redis-feast/deployments.yaml†L1-L120】【F:deploy/k8s/base/aether-services/deployment-capital-allocator.yaml†L1-L85】 |
