...
| Area | Status | Notes |
| --- | --- | --- |
| Architecture & Deployment | ✅ Ready | FastAPI TLS client secrets and the Secrets Service encryption key now reconcile from Vault-backed ExternalSecrets, unblocking the API rollouts, and Feast still targets the dedicated Redis master with complete credentials. The FastAPI settings reference the cluster DNS endpoints for Kafka, NATS, TimescaleDB, Redis, and Feast so pods connect to the shipped ports, and per-service cert-manager Issuers now mint the ingress `*-service-tls` secrets so HTTPS is enforced automatically across auth, fees, OMS, policy, risk, secrets, and universe. 【F:deploy/k8s/base/fastapi/deployments.yaml†L27-L114】【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L248】【F:deploy/k8s/base/secrets/fastapi-tls-external-secrets.yaml†L1-L140】【F:deploy/k8s/base/secrets/secrets-service-config-external-secret.yaml†L1-L16】【F:deploy/k8s/base/fastapi/configmap.yaml†L12-L34】【F:deploy/k8s/base/feast/external-secret.yaml†L1-L18】【F:deploy/k8s/base/feast/deployment.yaml†L33-L86】【F:deploy/k8s/base/feast/configmap.yaml†L9-L21】【F:deploy/k8s/base/redis-feast/deployments.yaml†L1-L120】【F:deploy/k8s/base/aether-services/deployment-capital-allocator.yaml†L1-L85】【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L132】【F:deploy/k8s/base/aether-services/ingress-auth.yaml†L1-L30】 |
...
* Cert-manager now manages both the legacy `aether-risk-tls` certificate and the per-service `risk-service-tls` secret via committed Issuer and Certificate manifests, so the risk ingress maintains HTTPS automatically. 【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L76】【F:deploy/k8s/base/aether-services/ingress-risk.yaml†L1-L31】
* Auth, fees, OMS, policy, risk, secrets, and universe ingresses all reference committed cert-manager Issuers and Certificates that mint their `*-service-tls` secrets, removing the manual TLS gap. 【F:deploy/k8s/base/secrets/tls-certificates.yaml†L17-L132】【F:deploy/k8s/base/aether-services/ingress-auth.yaml†L1-L30】【F:deploy/k8s/base/aether-services/ingress-fees.yaml†L1-L30】【F:deploy/k8s/base/aether-services/ingress-oms.yaml†L1-L30】【F:deploy/k8s/base/aether-services/ingress-policy.yaml†L1-L30】【F:deploy/k8s/base/aether-services/ingress-secrets.yaml†L1-L30】【F:deploy/k8s/base/aether-services/ingress-universe.yaml†L1-L30】
...
* Scaling controller alerts `scaling_controller_evaluations_stalled` and `scaling_gpu_pool_idle` now ship with the Prometheus rule set, matching the runbooks and on-call checklist expectations. 【F:docs/checklists/oncall.md†L11-L15】【F:deploy/observability/prometheus/prometheus.yaml†L151-L188】
