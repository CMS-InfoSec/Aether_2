...
* `fastapi-config` now targets the TLS endpoints for Kafka and NATS while continuing to reference the TimescaleDB, Redis, and Feast cluster DNS hostnames, so application clients negotiate encrypted streaming sessions end-to-end. 【F:deploy/k8s/base/fastapi/configmap.yaml†L12-L34】【F:deploy/k8s/base/kafka-nats/stack.yaml†L1-L141】
