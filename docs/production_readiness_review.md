...
| Area | Status | Notes |
| --- | --- | --- |
| Architecture & Deployment | ✅ Ready | Cert-manager now provisions ingress TLS via the reusable `letsencrypt-prod` ClusterIssuer and committed certificate manifests, completing automation for FastAPI and service endpoints. 【F:deploy/k8s/base/secrets/issuer.yaml†L1-L13】【F:deploy/k8s/base/secrets/tls-certificates.yaml†L1-L103】 |
...
* RMT-032 — Cert-manager issuer automation implemented for all TLS Certificates. Files: `deploy/k8s/base/secrets/`. Severity: Medium. Owner: Platform Ops. Status: Implemented.
