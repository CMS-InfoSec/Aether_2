...
| Security & Compliance | ✅ Ready | Network policies and Dockerfiles remain hardened, and Vault-backed ExternalSecrets now carry explicit owner labels plus vault path annotations for auditability while continuing to deliver the Kraken API credentials and Secrets Service key from GitOps. Kafka/Zookeeper listener hardening is tracked separately with compensating controls. 【F:deploy/k8s/networkpolicy.yaml†L1-L112】【F:deploy/docker/risk-api/Dockerfile†L1-L26】【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L120】【F:deploy/k8s/base/secrets/kraken-keys-external-secrets.yaml†L1-L44】【F:deploy/k8s/base/secrets/secrets-service-config-external-secret.yaml†L1-L21】【F:deploy/k8s/base/aether-services/deployment-oms.yaml†L92-L124】【F:deploy/k8s/base/kafka-nats/stack.yaml†L24-L182】【F:docs/runbooks/secrets-service-key-rotation.md†L1-L112】 |
...
* RMT-034 — Enforce Helm schema validation in CI so workloads cannot bypass required image, resource, and security controls. Files: `deploy/helm/aether-platform/values.schema.json`, `.github/workflows/lint-test.yml`. Severity: Medium. Owner: Platform Ops. Status: Mitigated. 【F:deploy/helm/aether-platform/values.schema.json†L1-L162】【F:.github/workflows/lint-test.yml†L33-L47】
