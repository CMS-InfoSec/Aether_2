...
* The base Kustomization now sources the `feast-offline-store` ExternalSecret exclusively from the Feast base, eliminating duplication while continuing to deliver the required credentials to the deployment. 【F:deploy/k8s/base/feast/external-secret.yaml†L1-L18】【F:deploy/k8s/base/feast/deployment.yaml†L33-L86】
...
* RMT-031 — Feast ExternalSecrets consolidated to eliminate duplication and drift. Files: `deploy/k8s/base/feast/external-secret.yaml`, `deploy/k8s/base/secrets/external-secrets.yaml`. Severity: Medium. Owner: Platform Ops. Status: ✅ Mitigated.
