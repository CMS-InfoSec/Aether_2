...
| Reliability & Observability | ✅ Ready | Grafana SLO dashboards now query the live `http_request_duration_seconds` and `risk_marketdata_latest_timestamp_seconds` series via committed datasources/dashboards provisioning, and a `kubectl port-forward` validation confirmed the latency and freshness panels render real data from Prometheus. OMS/websocket histograms and kill-switch metrics remain wired for alerting parity, and log rotation stays in place. 【F:deploy/observability/grafana/dashboards/slos.json†L1-L101】【F:deploy/observability/grafana/datasources.yaml†L1-L24】【F:deploy/observability/grafana/deployment.yaml†L1-L40】【F:deploy/observability/prometheus/prometheus.yaml†L33-L176】【F:deploy/observability/prometheus/configmap.yaml†L11-L48】【F:metrics.py†L538-L693】【F:services/kraken_ws_ingest.py†L40-L216】【F:drift_service.py†L149-L229】【F:kill_switch.py†L26-L175】 |
...
* RMT-037 — Validate Grafana SLO dashboards against live Prometheus metrics so operational reviews see policy/risk latency and market data freshness signals without manual query edits. Files: `deploy/observability/grafana/dashboards/slos.json`, `deploy/observability/grafana/datasources.yaml`, `deploy/observability/grafana/deployment.yaml`. Severity: Medium. Owner: Observability. Status: Verified.
