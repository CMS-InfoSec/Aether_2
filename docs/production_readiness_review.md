...
* RMT-043 — Ensure every ingress resource declares an `ingressClassName` (or Gateway binding) so traffic targets the intended controller. Files: `deploy/k8s/base/aether-services/ingress-*.yaml`, `deploy/k8s/base/ingress/ingress.yaml`. Severity: Medium. Owner: Platform. Status: Mitigated.
