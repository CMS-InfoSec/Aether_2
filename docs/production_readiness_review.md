...
| Area | Status | Notes |
| --- | --- | --- |
| Architecture & Deployment | ✅ Ready | Helm lint now enforces the chart schema in CI so required image pull policies, resource guarantees, and hardened security contexts remain in place alongside the ExternalSecret-driven service rollouts. 【F:.github/workflows/lint-test.yml†L33-L47】【F:deploy/helm/aether-platform/values.schema.json†L1-L162】【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L248】 |
...
| Security & Compliance | ✅ Ready | Kafka brokers now require SCRAM-SHA-512 SASL credentials delivered from Vault-backed ExternalSecrets, and the NATS deployment starts with Vault-sourced user/password authentication. FastAPI services mount the TLS bundle and consume the authenticated Kafka/NATS connection strings, closing the plaintext streaming gap. 【F:deploy/k8s/base/kafka-nats/stack.yaml†L1-L118】【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L439】【F:deploy/k8s/base/secrets/fastapi-secrets-external-secret.yaml†L1-L40】【F:deploy/k8s/base/fastapi/configmap.yaml†L1-L40】【F:deploy/k8s/base/fastapi/deployments.yaml†L1-L220】【F:deploy/k8s/base/fastapi/deployment-risk.yaml†L1-L80】【F:deploy/k8s/base/fastapi/deployment-ingestor.yaml†L1-L120】 |
...
* RMT-027 — Define certificates/ExternalSecrets for each ingress TLS secret (`auth|fees|oms|policy|risk|secrets|universe` `*-service-tls`) so cert-manager can provision HTTPS for every service. Files: `deploy/k8s/base/secrets/tls-certificates.yaml`, `deploy/k8s/base/secrets/issuer.yaml`. Severity: High. Owner: Platform. Status: Implemented.
...
* RMT-033 — ExternalSecret metadata ownership labels validated. Files: `deploy/k8s/base/secrets/external-secrets.yaml`, `deploy/k8s/base/secrets/fastapi-credentials-external-secret.yaml`. Severity: Medium. Owner: Security Platform. Status: Complete. 【F:deploy/k8s/base/secrets/external-secrets.yaml†L1-L120】【F:deploy/k8s/base/secrets/fastapi-credentials-external-secret.yaml†L1-L26】
...
* RMT-035 — Kafka/NATS authentication and encryption enforced through Vault-sourced SCRAM credentials and TLS mounts so streaming clients connect securely. Files: `deploy/k8s/base/kafka-nats/stack.yaml`, `deploy/k8s/base/secrets/external-secrets.yaml`, `deploy/k8s/base/secrets/fastapi-secrets-external-secret.yaml`, `deploy/k8s/base/fastapi/configmap.yaml`, `deploy/k8s/base/fastapi/deployments.yaml`, `deploy/k8s/base/fastapi/deployment-risk.yaml`, `deploy/k8s/base/fastapi/deployment-ingestor.yaml`. Severity: Critical. Owner: Security Platform. Status: Complete.
