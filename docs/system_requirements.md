...
* ✅ **Common adapters remain accessible even when pytest injects stubs.** Importing `services.common` now lazily proxies the adapter, config, and security helpers and reloads the real modules when a stub lacks the required attributes or even replaces the submodule before import, keeping the convenience imports available without breaking the test harness’s dependency injection.【F:services/common/__init__.py†L63-L166】
