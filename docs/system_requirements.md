...
Governance workflows record every sensitive action through the shared `SensitiveActionRecorder`,
which persists before/after snapshots via the Timescale-backed `TimescaleAuditLogger` and
`AuditLogStore`. The store wraps payloads in immutable `MappingProxyType` views and retries
failed writes so audit trails cannot be mutated or dropped while establishing a verifiable
sequence of actor, action, and correlation identifiers for governance reviews. The in-memory
`MetaGovernanceLog` simultaneously journals ensemble allocation decisions per symbol for the
Builder.io dashboard.

Auditors and directors access those records through the `/logs/export` endpoint, which enforces
auditor identity and streams the daily `audit_log`, fill, and PnL tables in JSON, CSV, PDF, or
Markdown formats. The exporter normalises timestamps, materialises immutable snapshots, and
uploads artefacts to object storage so the governance page and compliance teams can inspect
complete historical actions without direct database access.
