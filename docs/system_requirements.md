...
* ✅ **Authentication counters can be reinitialised.** The auth service now provides `_init_metrics`, rebuilding Prometheus counters against the shared registry by default while still accepting an optional override so metrics tests and runtime initialisation can reset state deterministically.【F:auth/service.py†L128-L200】
