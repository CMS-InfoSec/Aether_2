...
* ✅ **Common adapters load before pytest stubs.** Importing `services.common` eagerly initialises the adapters, config, and security helpers so the test-suite’s legacy module stubs can no longer shadow the real implementations, preventing the import-time `RedisFeastAdapter`/`ADMIN_ACCOUNTS` failures that previously halted collection.【F:services/common/__init__.py†L1-L63】
